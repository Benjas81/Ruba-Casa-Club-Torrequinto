...
// Pano2VR 6.1.2/17873
// Filename: Chihuahua CC Torrequinto menu texto con sombras.ggsk
// Generated 2026-07-31T17:22:07
...
function pano2vrSkin(player,base) {
	...
	this.addSkin=function() {
		...
		me._planta.logicBlock_scaling = function() {
			...
			if (me._planta.ggCurrentLogicStateScaling != newLogicStateScaling) {
				...
				if (me._planta.ggCurrentLogicStateScaling == 0) {
					...
					me._planta.ggParameter.sx = 0.6;
					me._planta.ggParameter.sy = 0.6;
					me._planta.style[domTransform]=parameterToTransform(me._planta.ggParameter);
				}
			}
		}
		me._planta.ggCurrentLogicStateScaling = -1;
		...
		el=me._clone_sala=document.createElement('div');
		el.ggNumRepeat = 1;
		el.ggNumRows = 0;
		el.ggNumCols = 0;
		el.ggWidth = 150;
		...
		el.ggFilter = [];
		el.ggInstances = [];
		me._clone_sala.callChildLogicBlocks_mouseover = function(){
			if(me._clone_sala.ggInstances) {
				var i;
				for(i = 0; i < me._clone_sala.ggInstances.length; i++) {
					if (me._clone_sala.ggInstances[i]._nodos_de_texto && me._clone_sala.ggInstances[i]._nodos_de_texto.logicBlock_textcolor) {
						me._clone_sala.ggInstances[i]._nodos_de_texto.logicBlock_textcolor();
					}
				}
			}
		}
		me._clone_sala.callChildLogicBlocks_active = function(){
			if(me._clone_sala.ggInstances) {
				var i;
				for(i = 0; i < me._clone_sala.ggInstances.length; i++) {
					if (me._clone_sala.ggInstances[i]._nodos_de_texto && me._clone_sala.ggInstances[i]._nodos_de_texto.logicBlock_textcolor) {
		...
		el.ggUpdate = function(filter) {
			if(me._clone_sala.ggUpdating == true) return;
			me._clone_sala.ggUpdating = true;
			var el=me._clone_sala;
			var curNumCols = 0;
			curNumCols = me._clone_sala.ggNumRepeat;
			...
			if ((el.ggNumCols == curNumCols) && (el.ggInstances.length > 0) && (filter.length === el.ggCurrentFilter.length) && (filter.every(function(value, index) { return value === el.ggCurrentFilter[index] }) )) {
				me._clone_sala.ggUpdating = false;
				return;
			} else {
				el.ggNumRows = 1;
			...
			for (var i=0; i < el.ggTagTable.length; i++) {
				...
				var parameter={};
				parameter.top=(row * me._clone_sala.ggHeight) + 'px';
				parameter.left=(column * me._clone_sala.ggWidth) + 'px';
				parameter.index=currentIndex;
				var inst = new SkinCloner_clone_sala_Class(nodeId, me, el, parameter);
				currentIndex++;
			...
			me._clone_sala.callChildLogicBlocks_mouseover();
			me._clone_sala.callChildLogicBlocks_active();
			me._clone_sala.ggUpdating = false;
			player.triggerEvent('clonerchanged');
			if (me._clone_sala.parentNode.classList.contains('ggskin_subelement') && me._clone_sala.parentNode.parentNode.classList.contains('ggskin_scrollarea')) me._clone_sala.parentNode.parentNode.ggUpdatePosition();
		}
		el.ggFilter = [];
		el.ggTagTable = [
			{tag:"Fachada",title:"Fachada"},
			{tag:"DogPark",title:"Dog Park"},
			{tag:"CasaClub",title:"Casa Club"},
			{tag:"InteriorCC",title:"Interior C. Club"},
			{tag:"FirePit",title:"Fire Pit"},
			{tag:"TerrazaCC",title:"Terraza C. Club"},
			{tag:"Asadores",title:"Asadores"},
			{tag:"PingPong",title:"Ping Pong"},
			{tag:"CanchaPickleball",title:"Cancha Pickleball"},
			{tag:"Estar",title:"Estar"},
			];
		el.ggId="clone sala";
		el.ggParameter={ rx:0,ry:0,a:0,sx:1,sy:1 };
		el.ggVisible=true;
		...
		el.setAttribute('style',hs);
		el.style[domTransform + 'Origin']='50% 50%';
		me._clone_sala.ggIsActive=function() {
			return false;
		}
		el.ggElementNodeId=function() {
			return player.getCurrentNode();
		}
		me._clone_sala.logicBlock_scaling = function() {
			var newLogicStateScaling;
			if (
				((player.getViewerSize().width < 600))
			)
			...
			if (me._clone_sala.ggCurrentLogicStateScaling != newLogicStateScaling) {
				...
				if (me._clone_sala.ggCurrentLogicStateScaling == 0) {
					...
					me._clone_sala.ggParameter.sx = 0.5;
					me._clone_sala.ggParameter.sy = 0.5;
					me._clone_sala.style[domTransform]=parameterToTransform(me._clone_sala.ggParameter);
				}
		...
		me._clone_sala.ggCurrentLogicStateScaling = -1;
		me._clone_sala.ggUpdateConditionNodeChange=function () {
			var cnode=player.getCurrentNode();
			for(var i=0; i<me._clone_sala.childNodes.length; i++) {
				var child=me._clone_sala.childNodes[i];
		...
		me._clone_sala.ggUpdatePosition=function (useTransition) {
				me._clone_sala.ggUpdate();
		}
		me._clone_sala.ggNodeChange=function () {
			me._clone_sala.ggUpdateConditionNodeChange();
		}
		me.divSkin.appendChild(me._clone_sala);
		...
		els.innerHTML="Casa Club<br\/>Torrequinto";
		el.appendChild(els);
		...
		me._titulo.onmouseover=function (e) {
			me.elementMouseOver['titulo']=true;
		}
		me._titulo.onmouseout=function (e) {
			if (e && e.toElement) {
				var current = e.toElement;
				while (current = current.parentNode) {
				if (current == me._titulo__text)
					return;
				}
			}
			me._titulo.style[domTransition]='none';
			me._titulo.style.opacity='1';
			me._titulo.style.visibility=me._titulo.ggVisible?'inherit':'hidden';
			me.elementMouseOver['titulo']=false;
		}
		me._titulo.ontouchend=function (e) {
			me.elementMouseOver['titulo']=false;
		}
		me._titulo.ggUpdatePosition=function (useTransition) {
		}
		me.divSkin.appendChild(me._titulo);
	...
	me.skinTimerEvent=function() {
		...
		if (me.elementMouseOver['titulo']) {
			me._titulo.style[domTransition]='none';
			me._titulo.style.opacity='0.5';
			me._titulo.style.visibility=me._titulo.ggVisible?'inherit':'hidden';
		}
	};
	...
	function SkinHotspotClass_ht_node(parentScope,hotspot) {
		...
		hs='data:image/svg+xml;base64,'+
	...
	function SkinCloner_clone_sala_Class(item, parentScope,ggParent,parameter) {
		var me=this;
		var hs='';
		me.parentScope=parentScope;
		me.ggParent=ggParent;
		...
		me.__div.setAttribute('style','position: absolute;width: 150px; height: 40px; visibility: inherit; overflow: visible;');
		...
		el=me._nodos_de_texto=document.createElement('div');
		els=me._nodos_de_texto__text=document.createElement('div');
		el.className='ggskin ggskin_textdiv';
		el.ggTextDiv=els;
		el.ggId="Nodos de texto";
		el.ggParameter={ rx:0,ry:0,a:0,sx:1,sy:1 };
		el.ggVisible=true;
		el.className="ggskin ggskin_text ";
		el.ggType='text';
		...
		el.appendChild(els);
		me._nodos_de_texto.ggIsActive=function() {
			if ((this.parentNode) && (this.parentNode.ggIsActive)) {
				return this.parentNode.ggIsActive();
			}
			return false;
		...
		me._nodos_de_texto.logicBlock_textcolor = function() {
			var newLogicStateTextColor;
			if (
				((me.elementMouseOver['nodos_de_texto'] == true)) || 
				((me._nodos_de_texto.ggIsActive() == true))
			)
			{
			...
			if (me._nodos_de_texto.ggCurrentLogicStateTextColor != newLogicStateTextColor) {
				me._nodos_de_texto.ggCurrentLogicStateTextColor = newLogicStateTextColor;
				me._nodos_de_texto__text.style[domTransition]='color 0s';
				if (me._nodos_de_texto.ggCurrentLogicStateTextColor == 0) {
					me._nodos_de_texto__text.style.color="rgba(255,255,127,1)";
				}
				else {
					me._nodos_de_texto__text.style.color="rgba(255,255,255,1)";
				}
			}
		}
		me._nodos_de_texto.onclick=function (e) {
			player.openNext("{"+me.ggNodeId+"}","");
		}
		me._nodos_de_texto.onmouseover=function (e) {
			me.elementMouseOver['nodos_de_texto']=true;
			me._nodos_de_texto.logicBlock_textcolor();
		}
		me._nodos_de_texto.onmouseout=function (e) {
			if (e && e.toElement) {
				var current = e.toElement;
				while (current = current.parentNode) {
				if (current == me._nodos_de_texto__text)
					return;
				}
			}
			me.elementMouseOver['nodos_de_texto']=false;
			me._nodos_de_texto.logicBlock_textcolor();
		}
		me._nodos_de_texto.ontouchend=function (e) {
			me.elementMouseOver['nodos_de_texto']=false;
			me._nodos_de_texto.logicBlock_textcolor();
		}
		me._nodos_de_texto.ggUpdatePosition=function (useTransition) {
		}
		me.__div.appendChild(me._nodos_de_texto);
	};
	me.addSkin();
	...
	me._planta.logicBlock_scaling();
	me._clone_sala.logicBlock_scaling();
	me._plantas.logicBlock_visible();
	me._plantas.logicBlock_alpha();
	player.addListener('sizechanged', function(args) { me._planta.logicBlock_scaling();me._clone_sala.logicBlock_scaling(); });
	player.addListener('fullscreenenter', function(args) { me._clone_sala.logicBlock_scaling(); });
	...
	player.addListener('changenode', function(args) { me._clone_sala.callChildLogicBlocks_active(); });
